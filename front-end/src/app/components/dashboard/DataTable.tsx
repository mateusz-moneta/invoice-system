...
const thead = (columns: string[]): React.JSX.Element[] =>
  columns.map(
    (column: string, index: number): React.JSX.Element => (
      <th
        key={index}
        className={'border-slate-600 font-medium p-4 pt-0 pb-3 text-slate-200 text-left'}
      >
        {column}
      </th>
    )
...
export default function DataTable(props: Props) {
  ...
  const [rows, setRows] = useState(
    makeRows(limitRows(props.rows, rowsPerPage, page), (_props: any) => props.rowComponent(_props))
  );
  const pagination = (action: 'plus' | 'minus') => {
    ...
    setPage(_page);
    setRows(
      makeRows(limitRows(props.rows, rowsPerPage, _page), (_props: any) =>
        props.rowComponent(_props)
      )
    );
  };
        ...
        <table className={'border-collapse table-auto w-full overflow-scroll'} id={props.pageName}>
          <caption className={'caption-bottom mt-3'}>
            ...
            <div className="inline-flex rounded-md shadow-sm ml-5" role="group">
              ...
              <DataTableButton link={`${props.pageName}/new`} last={true}>
                Add new
              </DataTableButton>
            </div>
          </caption>
          <thead>
            <tr>{thead(props.columns)}</tr>
          </thead>
          <tbody className={'bg-slate-800'}>{rows}</tbody>
        </table>
      </div>
    </div>
  );
